...
- `BRC38WalletData.parse` validates the required document, portable encodings, table closure, and
...
- `BRC39Envelope.parse` validates and splits the version 1 `WDAT` envelope before any password or
  memory-hard work. Configurable limits cap file size and untrusted Argon2id parameters.
